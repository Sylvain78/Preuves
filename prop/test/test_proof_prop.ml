...
let test_invalid_empty_proof _ =
        assert_raises ~msg:"Invalid empty proof" (Failure("Formula is not at the end of the proof")) (fun () -> prop_proof_verif ~hyp:[] ~proof:[] x1)

let test_invalid_proof _ =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x1,[x1])) (fun () -> prop_proof_verif ~hyp:[] ~proof:[(*TPPFormula*) x1] x1)

let test_invalid_proof_length_2 _ =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x1,[x1])) (fun () -> prop_proof_verif ~hyp:[] ~proof:[(*TPPFormula*) x1;(*TPPFormula*) x2] x2)

let test_invalid_proof_length_2_hyp _ =
        assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x2,[x1;x2])) (fun () -> prop_proof_verif ~hyp:[x1] ~proof:[(*TPPFormula*) x1;(*TPPFormula*) x2] x2)
...
let test_S1_Bourbaki _ =
  ...
  in
  assert_bool "test_S1_Bourbaki" (prop_proof_verif ~hyp:[] ~proof:[(*(*TPPFormula*)*) f] f)
