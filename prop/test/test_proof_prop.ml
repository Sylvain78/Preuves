...
let test_invalid_proof test_ctxt =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x1,[x1])) (fun () -> proof_verification ~hyp:[] ~proof:[x1] x1)

let test_invalid_proof_length_2 test_ctxt =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x1,[x1])) (fun () -> proof_verification ~hyp:[] ~proof:[x1;x2] x2)
...
let test_S1_Bourbaki test_ctxt =
  ...
  in
  assert_bool "test_S1_Bourbaki" (proof_verification ~hyp:[] ~proof:[f] f)
