...
let test_invalid_proof _ =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x1,[x1])) (fun () -> prop_proof_verif ~proof:[(*TPPFormula*) x1] x1)

let test_invalid_proof_length_2 _ =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x1,[x1])) (fun () -> prop_proof_verif ~proof:[(*TPPFormula*) x1;(*TPPFormula*) x2] x2)

let test_invalid_proof_length_2_hyp _ =
        assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x2,[x1;x2])) (fun () -> prop_proof_verif ~hypotheses:[x1] ~proof:[(*TPPFormula*) x1;(*TPPFormula*) x2] x2)
...
let test_S1_Bourbaki _ =
  ...
  in
  assert_bool "test_S1_Bourbaki" (prop_proof_verif ~axioms:!axioms_prop ~proof:[(*(*TPPFormula*)*) f] f)
