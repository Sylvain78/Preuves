...
type kernel_proof_term =
  ...
  | Th of int * (formula_prop  * formula_prop) list (*instantce of axiom i, with list of simulatenous substitution for params *)
...
let kernel_verif ?(axioms=[]) ?(theorems=[]) ?(hypotheses=[]) ~formula:f ~proof () =
  ...
  let formula_from_proof_term (axioms:Prop.Theorem_prop.theorem_prop list) 
    ...
    | Th (i,subst) ->
      ...
      let  lv,lt = List.split subst
      in
      Prop.Substitution_prop.simultaneous_substitution_formula_prop lv lt theorem.conclusion_prop
    | Cut(j,k) -> 
