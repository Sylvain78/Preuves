...
let test_invalid_empty_proof _ =
        assert_raises ~msg:"Invalid empty proof" (Failure("Formula is not at the end of the proof")) (fun () -> verif ~proof:[] ~formula:x1 ())

let test_invalid_proof _ =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x1, [], [], [x1])) (fun () -> verif ~proof:[x1] ~formula:x1)

let test_invalid_proof_length_2 _ =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x1, [], [], [x1])) (fun () -> verif ~proof:[x1;x2] ~formula:x2)

let test_invalid_proof_length_2_hyp _ =
  assert_raises ~msg:"Invalid proof" (Invalid_demonstration (x2, [], [x1], [x1;x2])) (fun () -> verif ~hypotheses:[x1] ~proof:[x1;x2] ~formula:x2)
