...
let rec simultaneous_substitution_formula_prop ~vars:lx ~terms:lt f  =
  let subst_aux  =  function
  ...
  let g = subst_aux f
  in
  Logs.debug (fun m -> m "[subst]%a"  Instance_notation_printers.pp_formula g);
  g
